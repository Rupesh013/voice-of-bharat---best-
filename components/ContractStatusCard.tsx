...
const statusConfig = {
  Pending: { badge: 'bg-yellow-100 text-yellow-800', label: 'Pending Approval' },
  Active: { badge: 'bg-blue-100 text-blue-800', label: 'Active (Growing)' },
  Harvesting: { badge: 'bg-indigo-100 text-indigo-800', label: 'Harvesting' },
  Completed: { badge: 'bg-green-100 text-green-800', label: 'Completed' },
  Disputed: { badge: 'bg-red-100 text-red-800', label: 'Disputed' },
};
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      <div className="flex justify-between items-start mb-4">
        <div>
          ...
          <p className="text-gray-500">with {contract.buyerName}</p>
        </div>
            ...
            {progressSteps.map((step, index) => (
              <React.Fragment key={step}>
                <div className="flex flex-col items-center">
                  ...
                  <p className={`text-xs mt-2 w-20 text-center ${index <= currentStepIndex ? 'font-semibold text-gray-800' : 'text-gray-500'}`}>{step}</p>
                </div>
